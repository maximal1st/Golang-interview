...
Стек (Stack) це структура даних, що працює за принципом "останній ввійшов, перший вийшов" (LIFO - Last-In, First-Out). Елементи додаються і видаляються з одного кінця, який називається вершиною стека.

Черга (Queue) це структура даних, що працює за принципом "перший ввійшов, перший вийшов" (FIFO - First-In, First-Out). Елементи додаються в одному кінці черги (звичайно називається "хвіст") і видаляються з іншого кінця (звичайно називається "голова").

Зв'язаний список (Linked List) це структура даних, що складається з вузлів, які містять дані та посилання на наступний вузол в списку. Він відрізняється від масиву тим, що елементи не зберігаються в послідовному блоку пам'яті, але можуть розташовуватися в різних місцях пам'яті.
...
Збалансоване дерево (Balanced Tree) це структура даних, де кожен вузол має не більше двох дочірніх вузлів, і різниця у висоті піддерев не перевищує певного обмеження. Це дозволяє забезпечити ефективні операції вставки, видалення та пошуку елементів. Відомі типи збалансованих дерев включають АВЛ-дерева, червоно-чорні дерева та B-дерева.
