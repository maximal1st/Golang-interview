...
Інтерфейс визначає набір методів, які повинен реалізувати тип даних, щоб задовольнити цей інтерфейс. Інтерфейси дозволяють створювати спільність між різними типами даних та реалізаціями, які різні за своєю природою та розширювати функціональність програмного коду, не змінюючи сам код, що його використовує.
...
Ось приклад структури проекту з Clean Architecture:
...
Docker — це платформа, яка дозволяє запускати додатки в ізольованих середовищах, відомих як контейнери. Контейнери забезпечують стандартизований спосіб упакування програмного забезпечення з усіма необхідними залежностями та конфігурацією. Docker дозволяє розробникам розгортати додатки на різних середовищах: безпосередньо на своєму робочому столі, локальному сервері або в хмарному середовищі.

Kubernetes — це система для оркестрації контейнерів, яка надає можливість автоматичного масштабування, керування, моніторингу та управління контейнеризованими додатками, дозволяючи легко керувати кластерами контейнерів, забезпечуючи високу доступність та розподіленість додатків.

Основна перевага використання Docker і Kubernetes полягає у спрощенні процесу розгортання, масштабування та керування додатками. Вони дозволяють забезпечити консистентність середовища від розробки до виробництва, полегшують тестування додатків та забезпечують ефективне використання ресурсів серверів.

Користувачі можуть використовувати Docker для контейнеризації своїх додатків, а потім Kubernetes для керування та масштабування цих контейнерів в розподіленому середовищі.

**12. Розкажіть про абстракції Kubernetes, з якими працювали?**

Kubernetes оперує певною кількістю абстракцій, що відображають стан системи: розгорнуті у контейнерах застосунки та робочі навантаження, пов'язані з ними ресурси мережі та диску, інша інформація щодо функціонування кластера.

Pod є найменшою і базовою одиницею розгортання в Kubernetes та використовується для запуску і масштабування контейнерів. Він представляє собою групу одного або кількох контейнерів, які спільно виконуються на одному вузлі, з власними мережевим простором, сховищами для обміну даними та унікальною IP-адресою.

Service є механізмом доступу до набору реплік Pod. Він створює сталий мережевий адресний простір, за допомогою якого можна звертатися до групи реплік Pod, незалежно від їх розміщення на конкретних вузлах. Service дозволяє масштабувати та надавати збалансований доступ до додатків.

Volume забезпечує механізм для зберігання даних в межах Pod. Він дозволяє контейнерам обмінюватися даними або зберігати дані, які зберігаються після закінчення життєвого циклу контейнера. Volume може бути підключений до одного або кількох контейнерів в Pod.

Namespace дозволяє групувати та ізолювати різні ресурси Kubernetes, створювати віртуальні кластери, які ділять одну фізичну інфраструктуру. Namespace використовується для організації та керування ресурсами в середовищах з багатьма користувачами або проектами.

Deployment визначає бажаний стан додатка та автоматично керує процесом розгортання та масштабування Pod. Deployment забезпечує декларативний спосіб опису стану додатка, а Kubernetes автоматично забезпечує його досягнення. Deployment дозволяє легко змінювати конфігурацію додатків та здійснювати безпечні оновлення.

**13. Що таке Pod? Як він влаштований?**

Pod є найменшою одиницею розгортання в Kubernetes. Він представляє собою один або декілька контейнерів, які спільно виконуються на одному вузлі (node) — фізичному або віртуальному сервері.

Кожен Pod в Kubernetes має свою унікальну IP-адресу в межах кластера і спільний простір імен (network namespace), що дозволяє контейнерам в одному Pod взаємодіяти один з одним за допомогою локального мережевого інтерфейсу. Контейнери в одному Pod можуть спільно використовувати томи, накопичувачі та інші ресурси, що спрощує комунікацію та спільне використання даних.

Усі контейнери в одному Pod спільно використовують однаковий контекст мережі, розклад та ізоляцію, що дозволяє їм працювати як єдине ціле. Наприклад, якщо є додаток, який складається з веб-сервера і бази даних, можна розгорнути обидва контейнери в одному Pod, щоб спростити їх взаємодію та забезпечити належну синхронізацію.

Pod є тимчасовим і працює як одиниця масштабування. Якщо потрібно більше ресурсів або необхідно збільшити кількість екземплярів додатку, Kubernetes може розгорнути додаткові копії Pod на різних вузлах у кластері.

Pod не зберігає стан. Якщо Pod перезапуститься або буде замінений новим, весь стан, збережений у контейнерах, буде втрачено. Однак можна використовувати інші ресурси, такі як PVC (Persistent Volume Claim), для збереження данних між перезапусками Pod.

**14. Розкажіть про Data structures: stack, queue, linked list, trie, balanced tree.**

Stack (Стек) це структура даних, що працює за принципом "останній ввійшов, перший вийшов" (LIFO — Last-In, First-Out). Елементи додаються і видаляються з одного кінця, який називається вершиною стека.

Queue (Черга) це структура даних, що працює за принципом "перший ввійшов, перший вийшов" (FIFO — First-In, First-Out). Елементи додаються в одному кінці черги (звичайно називається "хвіст") і видаляються з іншого кінця (звичайно називається "голова").

Linked list (Зв'язаний список) це структура даних, що складається з вузлів, які містять дані та посилання на наступний вузол в списку. Він відрізняється від масиву тим, що елементи не зберігаються в послідовному блоку пам'яті, але можуть розташовуватися в різних місцях пам'яті.

Trie (також відомий як префіксне дерево або регістрозалежне дерево) це структура даних, яка зберігає асоціативний масив, де ключі є рядками. Вона дозволяє ефективно шукати, вставляти і видаляти рядки, зокрема для задач, які пов'язані з операціями над текстом, пошуком словників, автодоповненням тощо.

Balanced Tree (Збалансоване дерево) це структура даних, де кожен вузол має не більше двох дочірніх вузлів, і різниця у висоті піддерев не перевищує певного обмеження. Це дозволяє забезпечити ефективні операції вставки, видалення та пошуку елементів. Відомі типи збалансованих дерев включають АВЛ-дерева, червоно-чорні дерева та B-дерева.

### Бази даних

**15. Які шляхи пошуку повільних SELECT-запитів у RDBMS? Які способи пришвидшення таких запитів?**

Пошук та вирішення проблем з повільними SELECT-запитами в реляційних базах даних може включати наступні кроки:

1. Відстеження часу виконання: засіб для відстеження часу виконання запитів може допомогти ідентифікувати запити, які вимагають надмірного часу для виконання. Це може бути вбудований механізм бази даних, зовнішній інструмент профілювання або логування.

2. Аналіз плану виконання: оцінка плану виконання запиту, який використовується оптимізатором бази даних, щоб переконатися, що оптимізатор використовує найкращі доступні індекси, виконує оптимальні об'єднання даних та використовує ефективні алгоритми для сортування та фільтрації.

3. Аналіз індексів: запити можуть бути повільними через відсутність або некоректне використання індексів. У разі необхідності слід додати потрібні індекси.

4. Перевірка наявності блокування: чи не блокують запит інші запити або транзакції, чи не виникають конфлікти блокування, такі як блокування на рівні таблиць або рядків.

5. Аналіз запитів та структури даних: чи не виконуються запити на великі обсяги даних, чи можна зменшити обсяги повернутих даних та чи можна використовувати більш ефективні структури даних для зберігання.

6. Оптимізація запитів: перевірка складності запитів, виконання з'єднань, підзапитів та інших операцій. Можливо, деякі запити можуть бути переписані або переструктуровані для покращення продуктивності.

7. Кешування даних: чи використовується кешування результатів запитів у пам'яті або іншому швидкодіючому сховищі. Це особливо корисно для запитів, які виконуються часто, але рідко змінюються.

8. Розділення завдань: чи не можна розділити запити на менші підзапити або завдання, щоб їх можна було виконувати паралельно або асинхронно. Це може поліпшити продуктивність, особливо в ситуаціях з великими обсягами даних та навантаженнями.

9. Масштабування та оптимізація інфраструктури: чи має сервер бази даних достатню обчислювальну потужність, пам'ять та пропускну здатність для існуючих вимог. У разі необхідності слід розглянути можливість масштабування бази даних, розподілу навантаження або використання кешування на рівні інфраструктури.

**16. Що таке нормальні форми бази даних?**

Нормальні форми бази даних є правилами, які визначають структуру та організацію даних у реляційних базах даних. Вони використовуються для забезпечення якості, цілісності та ефективності бази даних, допомагають уникнути аномалій даних, забезпечують ефективну організацію та зменшують дублювання даних. Існує кілька рівнів нормальних форм, які описують різні аспекти організації даних. Основними нормальними формами є:

1. Перша нормальна форма (1NF): вимагає, щоб кожне поле в базі даних містило атомарні значення, тобто значення, які не можна поділити на більші елементи. Крім того, усі пов'язані дані повинні бути збережені у відповідних таблицях, а кожен рядок у таблиці повинен бути унікально ідентифікований.

2. Друга нормальна форма (2NF): вимагає, щоб кожне поле в таблиці залежало від ключа таблиці, а не від часткових ключів. Це означає, що дані, які відносяться до одного ключа, повинні бути збережені у одній таблиці.

3. Третя нормальна форма (3NF): вимагає, щоб кожне поле в таблиці залежало від ключа таблиці та не залежало від інших неключових полів. Це допомагає уникнути виникнення залежностей між неключовими полями.

4. Четверта нормальна форма (4NF): вимагає, щоб жодна мультимножина залежностей не містила нетривіальну функціональну залежність між неключовими полями.

5. П'ята нормальна форма (5NF): вимагає, щоб кожен функціональний залежний рядок міг бути відновлений з інших залежностей за допомогою проекції.

**17. Що таке індекси? Які їхні недоліки?**

Індекси — це структури, які прискорюють процес пошуку та фільтрації даних. Вони створюються на певних стовпцях таблиць і дозволяють ефективно здійснювати операції пошуку та сортування.

Недоліками використання індексів є:

1. Використання місця: індекси займають додатковий простір у базі даних. Це призводить до збільшення розміру бази даних, особливо при створенні багатьох індексів.

2. Уповільнення запису: коли виконується операція запису в таблицю, індекси потребують оновлення. Це може вплинути на продуктивність операцій запису даних.

3. Потреба у підтримці: при змінах в базі даних індекси повинні підтримуватися та оновлюватися. Якщо дані часто змінюються або вставляються, це може вимагати додаткових зусиль для підтримки індексів у поточному та консистентному стані.

4. Збільшення часу на операції запису: оновлення індексів може займати додатковий час під час операцій запису даних. Чим більше індексів, тим більше часу може займати запис даних.

5. Обмеження на використання: індекси не підходять для всіх типів запитів. Іноді запити можуть вимагати складних або множинних умов, які не можуть бути оптимізовані за допомогою індексів.

**18. Які структури даних можуть використовуватися в індексах баз даних? Як вони працюють?**

Основні структури даних, що використовуються в індексах, включають:

1. Бінарні дерева пошуку (Binary Search Trees, BST): використовуються для швидкого пошуку даних за ключем. Вони забезпечують швидкий доступ до даних, проте їх продуктивність може залежати від балансування дерева.

2. Балансовані дерева пошуку (Balanced Search Trees): забезпечують балансування дерева для збереження оптимальної продуктивності пошуку. Вони є особливо корисними для індексів з великим обсягом даних. Сюди відносяться АВЛ-дерева, червоно-чорні дерева та B-дерева.

3. B+ дерева (B+ Trees): є розширенням B-дерев та зазвичай використовуються у базах даних для індексації. Вони забезпечують ефективний пошук та сортування даних, мають підтримку діапазонних запитів та підтримують високу щільність даних.

4. Хеш-таблиці (Hash Tables): використовують хеш-функції для швидкого пошуку значень за ключем. Вони забезпечують константний час доступу до даних, що робить їх дуже ефективними для пошуку. Проте, вони можуть мати обмежену сортувальну функціональність.

5. Хеш-дерева (Hash trees, Trie): використовуються для швидкого пошуку та індексації даних. Вони основані на структурі "дерева", де кожен шлях від кореня до листового вузла представляє ключове значення.

**19. Яка різниця між foreign та primary key?**

Primary key (Первинний ключ) — це унікальний ідентифікатор, який використовується для однозначної ідентифікації кожного запису в таблиці бази даних. Він служить основою для унікального доступу до даних в таблиці. Первинний ключ не може містити дублікатів або значень NULL. Кожна таблиця повинна мати первинний ключ, який однозначно ідентифікує кожен запис.

Foreign key (Зовнішній ключ) — це поле або набір полів, які посилаються на первинний ключ іншої таблиці. Він використовується для встановлення зв'язків між таблицями в базі даних. Зовнішній ключ дозволяє забезпечити цілісність даних та зв'язок між таблицями. Значення зовнішнього ключа повинні відповідати значенням первинного ключа в посиланій таблиці або бути NULL, якщо зв'язок не встановлено.

Отже, первинний ключ використовується для однозначної ідентифікації записів в таблиці, тоді як зовнішній ключ використовується для встановлення зв'язку між таблицями в базі даних.

**20. Поняття міграції у контексті баз даних. У чому переваги підходу з міграціями?**

Міграція баз даних — це процес зміни схеми бази даних або перенесення даних з однієї версії схеми до іншої, який включає створення, модифікацію або видалення таблиць, стовпців, індексів, обмежень та інших об'єктів бази даних. Міграції дозволяють здійснювати ці зміни керовано та контрольовано.

Переваги підходу з міграціями включають:

1. Керовані зміни схеми: міграції дозволяють змінювати схему бази даних керовано та систематично. Зміни описуються у вигляді коду міграції, що дозволяє легко відстежувати, контролювати та виконувати зміни.

2. Легкість розгортання та відкату: міграції можуть бути легко застосовані на базу даних для розгортання нових версій програмного забезпечення або відкату до попередніх версій. Це спрощує процес розгортання та дозволяє швидко відновлювати стан бази даних у разі необхідності.

3. Колективна робота: міграції дозволяють розробникам та командам спільно працювати над змінами схеми бази даних. Кожна міграція може мати свою власну версію та описувати конкретну зміну. Це полегшує спільну роботу, відстеження та злиття змін.

4. Автоматизація та повторюваність: міграції можуть бути автоматизовані та виконуватися повторювано. Це дозволяє виконувати зміни в базі даних на різних середовищах, таких як розробка, тестування та виробництво, забезпечуючи однакові результати.

5. Історія змін: міграції зберігають історію змін схеми бази даних. Це дозволяє відстежувати, коли і які зміни були внесені, а також використовувати їх для аналізу, документації та відладки.

Одже, підхід з міграціями дозволяє здійснювати контрольовані зміни в схемі бази даних, спрощує розгортання та управління змінами, забезпечує автоматизацію та повторюваність процесу. Він стає основою для ефективного управління схемою бази даних протягом життєвого циклу системи.

**21. Що таке реплікація даних? Які варіанти реплікації існують для баз, з якими ви працювали?**

Реплікація даних в базах даних — це процес створення та підтримки копій даних в різних вузлах або серверах з метою забезпечення доступності, надійності та швидкодії системи. Під час реплікації дані копіюються з одного джерела до одного або кількох призначених місць зберігання.

Існують різні варіанти реплікації, зокрема:

1. Master-Slave: один сервер виступає як первинний (master), а інші сервери — як вторинні (slaves). Майстер отримує всі записи та оновлення даних, а слейви отримують копії цих даних. Слейви можуть використовуватись для збільшення продуктивності читання (read scaling) та забезпечення кращої доступності. Однак, записи даних можуть здійснюватись лише на майстері.

2. Master-Master: обидва сервери виступають як майстери, і кожен з них може приймати записи та оновлення даних. Це дозволяє розподілити навантаження записів та оновлень між двома серверами і забезпечує більшу масштабованість записів. Проте, синхронізація даних між серверами може бути складнішою і вимагати додаткових механізмів для уникнення конфліктів.

3. Cluster: дані реплікуються на кілька серверів, що входять до кластеру. Кожен сервер в кластері має повний набір даних. Це забезпечує високу доступність, оскільки система може продовжувати працювати, навіть якщо один з серверів відмовить. Кластерна реплікація також може забезпечувати масштабованість записів та читань.

**22. Яка різниця між SQL і NoSQL?**

Різниця між SQL (Structured Query Language) і NoSQL (Not Only SQL) полягає в їхніх підходах до зберігання та обробки даних.

SQL бази даних використовують реляційну модель даних і мову запитів SQL для організації та маніпулювання даними. Основні риси SQL баз даних включають:

1. Структурованість: дані зберігаються в таблицях з фіксованою схемою, де кожен стовпець має тип даних.
2. Зв'язки: взаємозв'язки між таблицями встановлюються за допомогою ключів.
3. ACID властивості: SQL бази даних забезпечують транзакційну цілісність, використовуючи ACID властивості (атомарність, консистентність, ізольованість, довговічність).
4. Гнучкість запитів: SQL надає потужний мовний інтерфейс для запитів та маніпуляції даними, що дозволяє складати складні запити з використанням різних операцій.

NoSQL бази даних використовують різноманітні моделі даних, відмінні від традиційної реляційної моделі. Основні риси NoSQL баз даних включають:

1. Неструктурованість: дані можуть бути збережені в різних форматах, таких як документи, ключ-значення, стовпчикові чи графи.
2. Горизонтальне масштабування: NoSQL бази даних часто пропонують горизонтальне масштабування, що дозволяє розподілити навантаження на кілька серверів або кластерів.
3. Висока продуктивність: NoSQL бази даних зазвичай спрямовані на високу продуктивність та швидкий доступ до даних.
4. Гнучкість схеми: NoSQL бази даних дозволяють гнучку схему, що означає, що можна додавати та змінювати поля даних без необхідності зміни всієї схеми бази даних.

Обидва підходи мають свої переваги та недоліки, і вибір між SQL і NoSQL залежить від конкретних потреб. SQL бази даних зазвичай підходять для складних зв'язків та транзакційної цілісності, тоді як NoSQL бази даних підходять для швидкого доступу до великих обсягів нереляційних даних та гнучкої схеми.
...
Дивись [тут](#Column-oriented-DB).

**25. Що таке документоорієнтована БД? Переваги та недоліки.**

Дивись [тут](#Document-oriented-DB).

**26. Розкажіть про роботу з key-value базами даних (бажано з власного досвіду).**

Дивись [тут](#Key-value-DB) і [тут](#Redis).

**27. З якими типами даних Redis у вас є практичний досвід? Назвіть приклади, коли їх доцільно використовувати.**

Redis підтримує наступні типи даних:

1. Strings (рядки): це простий тип даних, де значення пов'язане з ключем.

    Використовуються для зберігання будь-якої довільної даних, таких як текст, числа або серіалізовані об'єкти.

2. Lists (списки): представляють собою впорядковані колекції значень, які можуть містити дублікати. Вони підтримують операції додавання, видалення та доступу до елементів за індексом.

    Використовуються для реалізації черг, стеків, зберігання логів подій, ведення журналів змін.

3. Sets (множини): є унікальними невпорядкованими колекціями значень без дублікатів. Вони підтримують операції додавання, видалення та перевірки членства.

    Використовуються для виконання операцій над множинами, таких як об'єднання, перетин, різниця тощо.

4. Hashes (хеші): представляють колекції пар ключ-значення, де ключі унікальні всередині хешу, а значення можуть бути рядками або числами. Хеші надають швидкий доступ до окремих елементів, а також підтримують операції зміни, видалення та отримання всіх значень.

    Використовуються для зберігання структурованих даних, кешування об'єктів, швидкого доступу до полів за ключем.

5. Sorted Sets (впорядковані множини): схожі на звичайні множини, але кожен елемент має асоційований з ним "рейтинг" або "сортувальне значення". Впорядковані множини використовуються для зберігання даних впорядкованими за цими значеннями, що дозволяє швидкий доступ до елементів за рангом або діапазоном значень.

    Використовуються для сортування даних за рейтингом або значеннями, ранжування, знаходження елементів за рейтингом або діапазоном значень.

### Go

**28. Якими бібліотеками Go ви користувалися для доступу до RDBMS? Які у них позитивні та негативні сторони?**

`go.dev/github.com/jmoiron/sqlx` це розширена версія пакету `database/sql`, яка надає додаткові функції для спрощення роботи з базами даних у Go. `sqlx` додає підтримку розширених операцій, таких як маппінг результатів SQL-запитів до структур даних, підтримку нульових значень, обробку NULL-значень, тощо. Цей пакет дозволяє використовувати переваги `database/sql` пакету, але з меншим обсягом коду. Позитивними сторонами `sqlx` є його простота використання, розширені можливості і підтримка баз даних різних вендорів. Недоліком є те, що він не надає високорівневих функцій і може вимагати більшого обсягу коду для виконання складніших операцій.

**29. Для чого використовують `Context`? Які є варіанти скасовування контекстів?**

Context у Go використовується для передачі та керування значеннями та поведінкою, які стосуються конкретного запиту або задачі, через різні горутини та сервіси. Він дозволяє передавати інформацію про таймаути, скасування операцій, значення налаштувань та інші контекстуальні дані, які потрібні для виконання конкретної операції або задачі.

Деякі важливі використання Context у Go включають:

1. Скасування операцій: Context дозволяє передавати сигнали про скасування та контролювати життєвий цикл операцій. Це особливо корисно, коли запити або операції мають обмеження за часом та необхідно перервати або очистити ресурси, якщо час очікування перевищено або користувачем скасовано операцію.

2. Передача значень та налаштувань: Context може містити значення, які потрібні різним компонентам або сервісам в процесі виконання операцій. Це дозволяє передавати налаштування, ідентифікатори сесій, інформацію про користувача та інші контекстуальні дані.

3. Передача метаданих: Context може містити метадані про операцію або запит, які можуть бути використані для логування, моніторингу, трасування запитів або статистики. Це дозволяє додатково налаштовувати та контролювати операції.

Щодо скасування контекстів, є кілька варіантів:

- `context.WithCancel` повертає новий контекст та функцію скасування (`cancel`). Виклик функції скасування призводить до поширення сигналу про скасування через всі об'єкти, які використовують цей контекст.

- `context.WithTimeout` або `context.WithDeadline` повертають новий контекст з таймаутом або дедлайном. Після закінчення таймауту або дедлайну контекст автоматично скасовується.

- `context.WithValue` дозволяє додавати значення до контексту, яке може бути отримане у всіх об'єктах, які використовують цей контекст. Це не скасовує контекст, але надає можливість передавати значення разом із контекстом.

**30. Якими способами можна виключити (приховати) поля структури при JSON-серіалізації?**

У Go є кілька способів виключити або приховати поля структури під час JSON-серіалізації:
...
1. Використання тегів JSON: у Go можна використовувати теги JSON для полів структури, щоб вказати їхню поведінку під час серіалізації. Використовуючи тег `json:"-"`, можна виключити поле з серіалізації.
У прикладі поле `Age` буде виключено з серіалізації, а поля `Name` та `Email` будуть включені.

2. Використання приватних полів: серіалізатор JSON в Go ігнорує приватні (непублічні) поля структури під час серіалізації. У прикладі поле `gender` буде виключено з серіалізації.

3. Використання інтерфейсу `json.Marshaler`: якщо потрібна більш гнучка логіка серіалізації, можна реалізувати інтерфейс `json.Marshaler` для структури. Це дозволить контролювати сам процес серіалізації та включати або виключати потрібні поля з вихідного JSON.

4. Тег `omitempty` вказує, що поле повинно бути виключене з серіалізації, якщо воно має значення за замовчуванням або нульове значення.

**31. Назвіть примітиви пакету `sync` стандартної бібліотеки. Яке призначення та приклади застосування `sync.WaitGroup`?**

Пакет `sync` в стандартній бібліотеці Go містить примітиви для синхронізації горутин і керування доступом до спільних ресурсів. Основні примітиви пакету `sync` включають:

- `sync.Mutex`: взаємовиключаючий м'ютекс (mutex), який забезпечує ексклюзивний доступ до спільних ресурсів. Використовується за допомогою методів `Lock()` та `Unlock()`.

- `sync.RWMutex`: блокування для читання-запису (read-write lock), яке дозволяє багато горутин одночасно читати спільні ресурси, але тільки одна горутина може записувати в ресурс. Використовується за допомогою методів `RLock()`, `RUnlock()`, `Lock()` та `Unlock()`.

- `sync.WaitGroup`: лічильник, який дозволяє чекати на завершення групи горутин. Використовується для синхронізації горутин, які працюють паралельно і потрібно дочекатись їх всіх, перш ніж продовжувати виконання. Програма, яка використовує `sync.WaitGroup`, може викликати метод `Add()` для додавання кількості горутин, які необхідно дочекатись, метод `Done()` для позначення завершення кожної горутини та метод `Wait()`, щоб зачекати, поки всі горутини завершаться.

**32. Яка різниця між Mutex та RWMutex?**

Різниця полягає у рівні доступу до спільних ресурсів:

`sync.Mutex` забезпечує ексклюзивний доступ до ресурсу для однієї горутини, тоді як інші горутини мають чекати.

`sync.RWMutex` дозволяє багато горутин одночасно читати ресурс, але тільки одна горутина може записувати. Читання не блокується і відбувається одночасно, але запис блокується, коли горутина бажає змінити ресурс.

**33. Які є способи зупинити N горутин, запущених одночасно (наприклад, worker pool)?**

Існує кілька способів зупинити N горутин, які працюють одночасно:

1. Використання сигнального каналу: створюється канал, який буде слухати кожна горутина. Коли потрібно зупинити горутини, відправляють сигнал через канал, і горутини перевірять цей сигнал та завершать свою роботу. Наприклад:
...
2. Використання контексту: використовується пакет `context` для створення контексту та передавання його кожній горутині. Коли потрібно зупинити горутини, контекст скасовується, і горутини отримають повідомлення про скасування та завершаться. Наприклад:
...
Замикання (closure) є комбінацією функції та лексичного оточення, в якому вона була оголошена. Замикання утворюється, коли функція звертається до змінних зовнішнього контексту, які зберігаються разом з функцією.

Замикання зберігають посилання на змінні зовнішнього контексту, навіть після того, як вони вийшли з області видимості, що дозволяє функції мати доступ до цих змінних навіть пізніше.

Замикання можуть модифікувати та змінювати значення змінних зовнішнього контексту, що дозволяє функції зберігати та оновлювати стан.

Замикання часто використовуються для створення функцій, які діють як функції вищого порядку, тобто функції, які приймають аргументами і повертають інші функції. Вони дозволяють створювати гнучкі та зручні конструкції програми, такі як ітератори, обробники подій, замикання над змінними та інші.

Ось приклад замикання в Go:
...
func main() {
	...
	greeting() // Виклик замикання
}
```

У цьому прикладі змінна `message` використовується у внутрішній функції `greeting`. Незважаючи на те, що змінна `message` вийшла з області видимості функції `main`, замикання все ще має до неї доступ та може її використовувати.

**35. Поясніть різницю між switch і select?**

`switch` використовується для перевірки значень змінної і виконання відповідного коду, в залежності від збігу з одним з варіантів.

`select` використовується для вибору та обробки різних комунікаційних операцій з каналами.

**36. Які є способи дістати дані з JSON?**

У мові програмування Go є кілька способів отримання даних з JSON:

1. Використання структури з тегами JSON: Ви можете оголосити структуру з полями, які відповідають полям у JSON-об'єкті. Використовуючи теги JSON, ви можете вказати, які поля JSON відповідають полям структури. Потім використайте функцію `json.Unmarshal` для розпакування JSON у структуру.
...
if err != nil {
    fmt.Println("Помилка розпакування JSON:", err)
    return
}

fmt.Println("Ім'я:", person.Name)
fmt.Println("Вік:", person.Age)
fmt.Println("Електронна пошта:", person.Email)
```

2. Використання типу `map[string]interface{}`: Якщо ви не знаєте структуру JSON наперед, ви можете використовувати тип `map[string]interface{}` для отримання даних з JSON. Використайте функцію `json.Unmarshal` для розпакування JSON у `map[string]interface{}`.
...
fmt.Println("Ім'я:", data["name"])
fmt.Println("Вік:", data["age"])
fmt.Println("Електронна пошта:", data["email"])
```

3. Використання декодера JSON: можна використати `json.NewDecoder` та `json.Decoder` для поступового декодування JSON-даних з потоку. Це корисно, коли є великий об'єм JSON-даних або потрібно обробити їх поступово без повного завантаження всього JSON-файлу в пам'ять.
...
Цикл `for` в Go може бути використаний для повторення блоку коду декілька разів. Його синтаксис має декілька варіантів:

`for i := 0; i < n; i++` — цикл зі змінною-лічильником, який повторюється n разів.
...
`for condition` — цикл з умовою, який повторюється, поки виконується певна умова.
...
`for range` — цикл для ітерації по колекціях (масивах, срічках, мапах тощо).
...
У мові Go немає окремого оператора `while`, але цикл `for` може бути використаний для побудови циклу `while` за допомогою одного умовного виразу.
...
У мові Go немає окремого оператора `do-while`, але цикл `for` може бути використаний для побудови циклу `do-while`, де перевірка умови здійснюється після виконання тіла циклу.
...
Крім того, в Go є додаткові ключові слова для управління циклами:

`break` використовується для виходу з циклу негайно.
`continue` використовується для переходу до наступної ітерації циклу.

**38. Як влаштований тип map?**

Тип `map` представляє собою структуру даних, яка забезпечує відображення ключів на значення і реалізоване у вигляді хеш-таблиці — структурі даних, яка забезпечує швидкий доступ до значень шляхом використання хеш-функції для обчислення індексу, за яким зберігається кожне значення.

Перед використанням `map` потрібно його ініціалізувати за допомогою функції `make`, яка виділяє пам'ять для `map`.

`map` є неупорядкованою колекцією, тобто порядок елементів в `map` не гарантований.

Ключі у `map` повинні бути унікальними. Якщо вставити значення з ключем, який вже існує в `map`, то воно замінить попереднє значення з цим ключем.

Значення у `map` можуть бути будь-якого типу, включаючи інші структури даних, вбудовані типи або користувацькі типи.

Доступ до значень у `map` здійснюється за допомогою ключів. Щоб отримати значення за ключем, використовується синтаксис `map[key]`.
...
fmt.Println(m["apple"]) // Виведе: 1

// Перевірка наявності значення для ключа
value, exists := m["banana"]
if exists {
    fmt.Println("Значення для ключа 'banana':", value) // Виведе: 2
}

// Видалення значення за ключем
delete(m, "apple")

// Перевірка наявності значення для ключа після видалення
_, exists = m["apple"]
if !exists {
    fmt.Println("Значення для ключа 'apple' відсутнє") // Виведе: Значення для ключа 'apple' відсутнє
}
```

**39. Який порядок перебору map?**

Порядок перебору елементів у `map` є недетермінованим. Це означає, що при кожному переборі може бути використаний інший порядок елементів.

Причина такого поведінки полягає у внутрішній реалізації хеш-таблиці, яка використовується для збереження `map` і не гарантує конкретного порядку збереження елементів та їх видачі при переборі.

**40. Що таке серіалізація? Де вона застосовується?**

Серіалізація — це процес перетворення структурованих даних в формат, який може бути збережений або переданий, зазвичай у вигляді бінарного потоку або текстового рядка і пізніше відновлений у вихідну структуру даних.

Серіалізація використовується для збереження структур даних у файлі, кеші чи базі даних, передачі через мережу між різними системами, міжпроцесної комунікації.

**41. Чи можна використовувати nil для ініціалізації змінної?**

Змінні типу `map`, `slice`, `pointer`, `interface`, `channel`, `function` можуть приймати значення `nil`. Коли змінна таких типів не має присвоєного значення, вона автоматично ініціалізується значенням `nil`.

Однак, змінні простих типів даних, таких як `int`, `float`, `bool`, `string`, не можуть бути прямо ініціалізовані значенням `nil`, оскільки вони є значимими типами і не допускають значення `nil` в якості свого значення. Замість цього, їхні значення за замовчуванням будуть мати "нульове" значення, наприклад, `0` для числових типів або порожній рядок для типу `string`.

**42. Чи можна задати місткість map? Чи можна отримати місткість map?**

Немає необхідності і можливості прямо задавати місткість `map`. Вона визначається автоматично під час додавання елементів до `map` і змінюється динамічно відповідно до потреб.

Поточну кількість елементів у `map` за допомогою функції `len`, яка повертає кількість пар ключ-значення у `map`.
...
// Отримання місткості map
capacity := len(m)
fmt.Println(capacity) // Виведе: 2
```
...
Кодогенерація (автогенерація коду) — це процес автоматичного створення вихідного коду на основі певних шаблонів, правил або метаданих. Використовується для автоматизації створення повторюваного або рутинного коду, що може бути монотонним і витрачати багато часу.

Кодогенерація може бути застосована для генерації коду на основі специфікацій або моделей, створення CRUD-операцій, генерації серіалізаторів і десеріалізаторів (JSON, XML або Protobuf), автоматичної генерації тестів, створення структур даних і алгоритмів, генерації бізнес-логіки.

Основна перевага кодогенерації полягає в тому, що вона дозволяє економити час розробки, забезпечує однорідність коду, знижує ризик помилок і сприяє швидкому впровадженню змін.

**45. Чим відрізняється goroutine від OS thread?**

Goroutine — це легковажна, недорога у використанні одиниця паралелізму, створена на рівні програми. В порівнянні з OS thread, яка є важкою і вимагає багато ресурсів, goroutine використовує значно менше пам'яті та ресурсів операційної системи.

Управління плануванням горутинами відбувається на рівні самої мови програмування, а не операційної системи. Система планування Go виконує розподіл горутин на доступні потоки OS threads. Це дозволяє більш ефективно управляти виконанням горутин, забезпечувати конкурентність та автоматично розподіляти ресурси.

**46. Як і для чого використовують ``io.Reader`` і ``io.Writer``?**

``io.Reader`` і ``io.Writer`` є інтерфейсами стандартної бібліотеки Go, які використовуються для читання та записування даних. Вони надають уніфікований спосіб роботи зі вхідними та вихідними потоками даних без необхідності залежати від конкретної реалізації читання або запису.
...
func main() {
	...
	// Створюємо новий зріз []io.Reader
	readers := make([]io.Reader, len(readWriters))

	// Зчитуємо з кожного io.ReadWriter в io.Reader
	for i, rw := range readWriters {
		readers[i] = rw.(io.Reader)
	}

	// Використовуємо зріз io.Reader
	for _, reader := range readers {
		// Виконуємо операції зчитування з кожного reader
		// ...
	}
}
```
...
У Go для організації синхронізації та очікування завершення роботи інших горутин можна використовувати пакет `sync` і його тип `sync.WaitGroup`, яктий надає механізм для очікування завершення певної кількості горутин.
...
func worker(id int, wg *sync.WaitGroup) {
	...
	time.Sleep(time.Second) // Імітація роботи горутини
...
func main() {
	...
	// Запуск 3 робочих горутин
	for i := 1; i <= 3; i++ {
		wg.Add(1)
		go worker(i, &wg)
	}

	wg.Wait() // Очікування завершення всіх робочих горутин
...
Якщо обсяг даних малий і займає кілька байтів, передача їх як значення може бути ефективнішою. При передачі значення функція отримує копію даних, що може бути швидше, оскільки немає потреби у вирахуванні адреси вказівника та доступі до значення через нього.

Якщо дані великі або структуровані, або якщо потрібно модифікувати значення змінної всередині функції і зберегти ці зміни поза функцією, передача вказівника є доцільною.

**50. Що таке варіативна змінна функції? Як працювати з цією змінною?**

Варіативна змінна використовується у функції, яка приймає довільну кількість аргументів, і представлена за допомогою трьох крапок "..." перед типом останнього аргументу функції.

Оскільки аргументи, представлені варіативною змінною, передаються у вигляді зрізу, до неї можна застосовувати різноманітні операції над зрізами, такі як ітерація, отримання розміру зрізу, безпосередній доступ до елементу.
...
result2 := sum(10, 20)       // result2 = 30
```

**51. Як працювати з пакетом ``internal``?**

Пакет `internal` призначений для організації внутрішньої структури програми і обмеження доступу до певних компонентів коду.

Пакет, розташований в директорії `internal`, доступний тільки для імпорту в межах батьківського модуля.

**52. Як працює імпорт через крапочку і чому це погана практика?**

Імпорт через крапочку використовується для прямого доступу до ідентифікаторів пакету (типів, функцій, констант і т. д.) без необхідності вказувати ім'я пакету перед кожним ідентифікатором.

```
import . "fmt"

func main () {
    Println("Some info...") // fmt.Println()
}
```

Хоча імпорт через крапочку може здатися зручним, його використання не рекомендується і вважається поганою практикою. Ось кілька причин:

- приховує походження функцій та інших елементів, які використовуються у коді, що може ускладнити розуміння, особливо у великих проектах з багатьма пакетами;

- може створювати конфлікти імен, особливо якщо імпортовані пакети мають експортовані імена, які конфліктують з іменами в поточному пакеті;

- може робити код менш зрозумілим і важчим для аналізу неявних залежностей, що ускладнює визначення, які пакети використовуються у проекті;

- код стає більш вразливим до змін у пакетах, які імпортуються. Якщо в пакеті змінюється експортоване ім'я або структура під час оновлення пакету, це може призвести до збоїв.

**53. Як працює імпорт через підкреслення?**

Імпорт через підкреслення використовується для того, щоб імпортувати пакет без прямого використання його ідентифікаторів у коді з метою виконання ініціалізаційних функцій або реєстрації функціональності.

Наприклад, можна імпортувати пакет `database/sql` через підкреслення, щоб виконати ініціалізацію драйверів бази даних, але не використовувати безпосередньо у коді функції або типи, що надає цей пакет.
...
`defer` — це ключове слово, яке використовується для відкладеного виконання функцій. Коли виклик функції відбувається через `defer`, його буде відкладено до кінця поточної функції тобто після виконання решти логіки функції, незалежно від того, яким чином функція була завершена.
...
func processFile(filename string) error {
    ...
    defer file.Close()  // Виклик file.Close() буде відкладено до кінця функції
...
В даному прикладі, незалежно від того, як функція `processFile` буде завершена (успішно чи з помилкою), виклик `file.Close()` буде виконано, що забезпечить правильне закриття файлу і звільнення ресурсів.

**55. Як працювати з goto?**

Ключовим слово `goto` використовується для безумовного переходу до певної мітки у програмі. Однак його використання є практикою, яку рекомендується уникати, оскільки вона може зробити код складним для розуміння та підтримки, викликати неконтрольований потік виконання програми та збільшити ймовірність помилок.
...
Пакет `reflect` надає можливості для інтроспекції (вивчення властивостей об'єктів під час виконання програми) і рефлексії (здатність програми аналізувати і змінювати свою структуру та поведінку).

`reflect.DeepEqual()` використовується для порівняння значень на рівні глибокого порівняння, а `reflect.TypeOf()` використовується для отримання типу об'єкту.
...
func main() {
	...
	fmt.Println(typ.Name()) // Person
}
```

**57. Як розпарсити час?**

Для розбору часу можна використовувати пакет `time`. Основним методом для розпарсування рядка в час є функція `time.Parse()`:
...
func main() {
	...
	str := "11.05.2023 12:30:45"

	parsedTime, err := time.Parse(layout, str)
	if err != nil {
		fmt.Println("Помилка під час розбору часу:", err)
		return
	}
...
Для порівняння двох дат можна використовувати методи типу `time.Time`, такі як `Before()`, `After()` та `Equal()`:
...
func main() {
	// Створення двох об'єктів типу time.Time
	time1 := time.Date(2023, time.May, 11, 12, 0, 0, 0, time.UTC)
	time2 := time.Date(2023, time.May, 10, 12, 0, 0, 0, time.UTC)

	// Порівняння дат
	if time1.Before(time2) {
		fmt.Println("time1 перед time2")
	} else if time1.After(time2) {
		fmt.Println("time1 після time2")
	} else {
		fmt.Println("time1 та time2 однакові")
	}
}
```
...
Вказівник є змінною, яка зберігає адресу пам'яті іншої змінної та дозволяє отримати доступ до значення змінної за її адресою.

Використання вказівників дозволяє ефективно маніпулювати даними, особливо в випадках, коли потрібно передавати великі структури даних або змінювати значення змінних без копіювання.
...
func main() {
	...
	// Присвоєння вказівнику адреси пам'яті змінної x
	p = &x

	// Виведення значення вказівника та значення, на яке він посилається
	fmt.Println("Значення x:", x)
	fmt.Println("Адреса x:", &x)
	fmt.Println("Значення, на яке посилається p:", *p)
	fmt.Println("Адреса, на яку посилається p:", p)

	// Зміна значення через вказівник
	*p = 20
	fmt.Println("Нове значення x:", x)
}
```
...
if _, ok := myVar.(MyInterface); ok {
    // Змінна myVar імплементує інтерфейс MyInterface
} else {
    // Змінна myVar не імплементує інтерфейс MyInterface
}
```

**61. Що таке embedding?**

Вбудовування дозволяє створювати ієрархію типів з використанням композиції, замість успадкування, що сприяє більш гнучкій організації коду і розширенню його функціональності.

Поля і методи вбудовуваного типу стають доступними для вбудовуючого типу без необхідності явного звернення до вбудованого типу.
...
func main() {
	emp := Employee{
		Person:  Person{Name: "John", Birthday: time.Date(1993, time.May, 29, 23, 0, 0, 0, time.UTC)},
		Company: "ABC Corp",
		Salary:  5000.0,
	}

	fmt.Println(emp.Name)   // Доступ до поля вкладеної структури
	fmt.Println(emp.Age())  // Доступ до метода вкладеної структури
	fmt.Println(emp.Salary) // Доступ до поля зовнішньої структури
}
```

**62. Опишіть кроки процесу тестування.**

Основні кроки процесу тестування можуть включати наступні етапи:

1. Аналіз вимог: розуміння вимог до програмного продукту і його функціональності. Це включає читання специфікацій, вимогових документів або комунікацію зі зацікавленими сторонами, щоб отримати чітке уявлення про очікувану поведінку системи.

2. Планування тестування: визначення стратегії тестування, складання плану тестування та визначення необхідних ресурсів, включаючи людські ресурси, середовища тестування та дані для тестування.

3. Розробка тестових сценаріїв: створення набору тестових сценаріїв або тестових кейсів, які покривають різні аспекти функціональності програмного продукту. Це може включати введення вхідних даних, виконання послідовності дій та перевірку очікуваних результатів.

4. Виконання тестів: запуск тестів відповідно до розроблених тестових сценаріїв або кейсів. Це може включати ручне виконання тестів або автоматизоване тестування за допомогою спеціальних інструментів.

5. Запис результатів тестування: фіксація результатів тестів, включаючи успішні та невдачні тести, виявлені проблеми або дефекти, а також відстеження стану виконання тестів.

6. Аналіз результатів: оцінка результатів тестування, виявлення проблем або дефектів та їх класифікація за важкістю або пріоритетом. Визначення причин проблем та пошук шляхів для виправлення.

7. Документування: запис результатів тестування, створення звітів, документації тестових сценаріїв та кейсів, а також будь-якої іншої необхідної документації, пов'язаної з процесом тестування.

8. Повторне виконання тестів: у випадку виправлення проблем або дефектів проводиться повторне виконання тестів для перевірки, що виправлення проблеми не впливає на інші аспекти програмного продукту.

9. Валідація та верифікація: перевірка, що програмний продукт відповідає вимогам і функціональності, а також визначення відповідності до стандартів і нормативних вимог.

**63. Як писати тести? Що таке табличні тести?**

Основна мета тестування — перевірити, чи виконує програмний код очікувані результати і чи працює він правильно у різних сценаріях:

1. Визначити вимоги до функціональності, яку треба протестувати. Це допоможе зорієнтуватися при написанні тестових сценаріїв.

2. Вибрати тестові сценарії, які покривають різні аспекти функціональності програмного продукту, розглянути різні вхідні дані, граничні значення та непередбачувані умови.

3. Написати тестові кейси для кожного сценарію, описвти вхідні дані, кроки виконання і очікувані результати з використанням чіткої та зрозумілої нотацію для опису тестових кейсів.

4. Написати код для виконання тестів, використовуючи тестові фреймворки або бібліотеки, які допомагають управляти тестами і перевіряти результати.

5. Запустити тести і перевірити їх результати, переконатися, що всі тести пройшли успішно і очікувані результати співпадають з отриманими.

6. Якщо тести не пройшли успішно, використати результати тестування для пошуку помилок і дефектів, виправити проблеми і повторно запустити тести, щоб перевірити, чи вони вирішені.

Табличні тести — це методика тестування, в якій використовується таблиця для опису різних варіацій вхідних даних і очікуваних результатів. Кожен рядок таблиці представляє окремий тестовий сценарій зі своїми вхідними даними і очікуваними результатами. Це дозволяє описати і виконати велику кількість тестів з різними комбінаціями даних. Табличні тести допомагають забезпечити повноту тестування і покриття різних варіантів виконання програми.

**64. Що таке memory leak? Які є способи його виявлення? Як його позбутися?**

В Go виток пам'яті є рідкісним явищем завдяки вбудованій системі збору сміття (garbage collection), яка автоматично вивільняє пам'ять, яку більше не використовує програма.

Однак, деякі практики можуть призводити до небажаних витоків пам'яті: несвоєчасний виклик функції `Close()` або аналогічної для об'єктів, які вимагають вивільнення пам'яті, таких як файли, мережеві з'єднання тощо, зациклення посилань між об'єктами, використання великих буферів або структур даних тощо.

Є кілька інструментів, які допомагають виявляти витоки пам'яті та працювати з пам'яттю ефективно. Ось деякі з них:

1. Пакет `runtime` надає функціонал для контролю над використанням пам'яті та виявленням витоків. Наприклад, функція `runtime.GC()` викликає збірку сміття для вивільнення непотрібної пам'яті. Функція `runtime.ReadMemStats()` дозволяє отримати детальну інформацію про використання пам'яті.

2. Інструменти профілювання, такі як `go tool pprof`, які дозволяють аналізувати використання пам'яті та виявляти проблемні місця.

3. Пакети для тестування, такі як `testing`, або сторонні інструменти для профілювання та моніторингу використання пам'яті.

4. Сторонні інструменти, такі як `go-torch`, `go-memprof`, `pprof`, які допомагають виявляти витоки пам'яті та аналізувати використання пам'яті в більш детальному режимі.

5. Використання рекомендації та практики, які допомагають уникати витоків пам'яті, таких як використання пулів буферів, відкриття та закриття файлів відповідно, використання каналів та синхронізаційних примітивів.

**65. Що таке race condition? Які є способи його виявлення? Як його позбутися?**

Race condition — це стан, коли поведінка програми залежить від того, які потоки або горутини виконуються в якому порядку та з якою швидкістю. У такому випадку результати виконання можуть бути непередбачуваними та некоректними.

Race condition виникає, коли два або більше потоки (або горутини) одночасно змінюють спільні дані без синхронізації. Це може статися, наприклад, коли кілька потоків одночасно звертаються до однієї змінної, яку вони читають та змінюють.

Існує кілька способів виявлення race condition:

1. Тестування з різними комбінаціями потоків або горутин та аналіз результатів виконання. Виявлення неправильних результатів або непередбачуваної поведінки може свідчити про наявність race condition.
...
3. Атомарні операції для змінення спільних даних, які гарантують, що операція виконується безпосередньо, без можливості переривання.

4. Ізоляція даних, як от розділення або копіювання, щоб кожен потік або горутина мала власний набір даних для роботи. Це дозволяє уникнути конфліктів при одночасному доступі до спільних даних.
...
Дивись [тут](#Processes-and-threads).

Процеси мають власні виділені області пам'яті і не можуть просто так звертатися до пам'яті іншого процесу. Це робить модель процесу безпечнішою і дозволяє уникнути конфліктів у використанні пам'яті.

Потоки, які належать до одного процесу, мають спільну область пам'яті, включаючи змінні та дані. Це може викликати конкуренцію у доступі до спільних ресурсів.
...
Saga — патерн проєктування, який дозволяє координувати транзакції здійснені до різних баз даних у вигляді однієї операції. Транзакції виконуються одна за одною і в разі невдачі хоч однієї скасовуються усі попередні.
